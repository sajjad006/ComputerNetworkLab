...
string injectError(string frame, ErrorType type) {
    ...
    switch (type) {
        ...
        case ODD_ERRORS: {
            ...
            if (maxOdd <= 1) { flipBit(frame, 0); break; }
            int numErrors = 1 + 2 * (rand() % ((maxOdd + 1) / 2));
            vector<int> positions;
            while (static_cast<int>(positions.size()) < numErrors) {
                int p = rand() % totalBits;
                if (find(positions.begin(), positions.end(), p) == positions.end()) {
        ...
        case BURST: {
            ...
            int L = (minBurstLength == maxBurstLength)
                        ? minBurstLength
                        : minBurstLength + rand() % (maxBurstLength - minBurstLength + 1);
            int start = rand() % (totalBits - L + 1);
            for (int i = 0; i < L; i++) flipBit(frame, start + i);
            break;
        }
    }
    return frame;
